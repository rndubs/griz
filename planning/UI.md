...
- 🟡 `ui/MainWindow`: menubar (File / Edit / View / Draw / Select / Animate / Window / Help); every menu action routes through `Console::execute`. *(Menubar structure shipped, plus accessors (`console()`, `materialsDock()`) and `setConnectionStatus` / `setHostLabel` / `setStateSeqLabel` / `setFpsLabel` slots used by `App`. Per-action wiring still ⬜.)*
...
- 🟡 `commands/CommandBridge`: drag → `rx`/`ry`/`tx`/`ty`, scroll → `zoom`, click → `pick_at`, `R` → `rview`; one-command-in-flight rate limit. *(`client/src/commands/CommandBridge.{h,cpp}` new subtree; installs an event filter on `Viewport`. Left drag → `rx`/`ry` at 0.25 deg/px; `Shift`+left or middle drag → `tx`/`ty` at 0.002/px; wheel → `zf`/`zb` by `1.10^notches`; release within 3 px of press → `pick_at <x> <y> any`; R key → `rview`. One command in flight: mouse-move deltas accumulate into a pending coalesced vector that flushes on response; wheel/click/R drop-newest while unacked. Dominant-axis picker per flush sends either `rx` or `ry` (not both) so the non-dominant delta survives to the next idle tick. Build: `griz-client-commands` static lib linked into the executable.)*
- 🟡 Docks: `MaterialsDock`, `TimeSlider`, `ResultsDock`, `SelectionDock` wired to `SessionState` signals via `Qt::QueuedConnection`. *(All four docks now read from `SessionState`. `MaterialsDock` — `QListWidget` of id + label + color swatch + `[hidden]`/`[disabled]` flags via `materialsChanged`. `TimeSlider` — horizontal `QSlider` bound to 1-based `state_min..state_max` with a `state N / M  t=X` label; user moves emit `stateRequested(N)` → `Worker::sendCommand("state N")`, and server-driven `timeChanged` events mirror back with `m_suppressEmit` to break the loop (slider has `tracking=false`, so scrubbing sends only on release). `ResultsDock` — `QFormLayout` with primary / component / griz_name / min / max, hides all rows and shows a placeholder when `hasActive=false`. `SelectionDock` — per-kind element + node `QListWidget`s with counts and a `hilite: one object`/`(none)` header, driven by `selectionChanged`.)*
- 🟡 Status bar: connection state, FPS from `frame_seq` deltas, host nickname, current `state_seq`. *(Connection label transitions through `connecting…` → `connected` → `disconnected` driven by `Worker::connected`/`disconnected`; `seq=N` updates from `state_changed` via `SessionState::lastSeq()`; FPS is live — `Viewport` records each binary-frame arrival time and emits a rolling 30-sample estimate on `fpsChanged`, which `MainWindow::setFpsLabel` renders. Host nickname is still hardcoded `"local"` pending Phase 6 host profiles.)*
