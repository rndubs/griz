...
- 🟡 Docks: `MaterialsDock`, `TimeSlider`, `ResultsDock`, `SelectionDock` wired to `SessionState` signals via `Qt::QueuedConnection`. *(`MaterialsDock` is now a `QListWidget` populated from `SessionState::materialsChanged()` — id + label, color swatch, `[hidden]`/`[disabled]` flags — driven by the `q_state` round-trip after `Worker::connected()`. The other three docks still pend their model wiring.)*
- 🟡 Status bar: connection state, FPS from `frame_seq` deltas, host nickname, current `state_seq`. *(Connection label transitions through `connecting…` → `connected` → `disconnected` driven by `Worker::connected`/`disconnected`; `seq=N` updates from `state_changed` via `SessionState::lastSeq()`. FPS from frame deltas still pends the Phase 3 mailbox split.)*
