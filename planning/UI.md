...
- ⬜ `net/Worker` + `net/Framing`: length-framed read/write, hello/hello_ack, request-id correlation with per-id waiter + timeout. *(Headers shipped as compile-clean stubs; wire bring-up is the next Phase-5 commit. See `client/src/net/`.)*
- ⬜ `model/SessionState`: in-memory mirror of `q_state`; apply `state_changed` diffs; `stateOverflow()` signal triggers a full refetch — [04-client.md §4](ui-design/04-client.md). *(Class + POD structs shipped as stubs; `applyFullState` / `applyDiff` are no-ops pending Worker.)*
- 🟡 `ui/MainWindow`: menubar (File / Edit / View / Draw / Select / Animate / Window / Help); every menu action routes through `Console::execute`. *(Menubar structure landed with the eight empty top-level menus; action-to-Console wiring pends Worker. See `client/src/ui/MainWindow.cpp`.)*
- 🟡 `ui/Viewport`: `QOpenGLWidget` that blits received frames; resize-to-server throttled to 250 ms quiescence; neutral-background fallback when disconnected. *(`QOpenGLWidget` subclass with the neutral-background + overlay-label fallback landed; frame upload and resize throttle pend Worker.)*
- 🟡 `ui/Console`: prompt + output pane; in-session Up/Down history; inline error rendering with `error.code` badge. *(Prompt + output + history landed as a `QWidget` embedded in a bottom `QDockWidget`; `Console::execute` echoes locally and emits `commandEntered`. Error-badge rendering pends Worker's `Response` signal.)*
- ⬜ `commands/CommandBridge`: drag → `rx`/`ry`/`tx`/`ty`, scroll → `zoom`, click → `pick_at`, `R` → `rview`; one-command-in-flight rate limit.
- 🟡 Docks: `MaterialsDock`, `TimeSlider`, `ResultsDock`, `SelectionDock` wired to `SessionState` signals via `Qt::QueuedConnection`. *(All four docks landed as `QDockWidget` subclasses with placeholder children in the default layout — Materials top-left, Selection bottom-left, Results top-right, Time bottom-right. Model wiring pends `SessionState` implementation.)*
- 🟡 Status bar: connection state, FPS from `frame_seq` deltas, host nickname, current `state_seq`. *(Four permanent-widget labels shipped with placeholder text; live updates pend events.)*
