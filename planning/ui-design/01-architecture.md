...
`griz-client` runs three threads. The **Qt UI thread** owns all widgets and the event loop and never performs network or decode work. The **network thread** owns the RPC socket and the SSH child process handle. (Post-MVP, it also owns the reconnect state machine; v1 has no reconnect — see Invariant I12 — so the network thread simply tears down the session on socket close.) The **decode thread** pulls frame payloads from the network thread, decodes to RGBA, and posts to the UI thread via a Qt queued connection. Input events are encoded on the UI thread and dropped into the network thread's send queue without blocking.
...
- **SSH drop.** Network thread detects tunnel death; UI shows a "Connection lost — relaunch?" modal. In v1 the server has already exited (Invariant I12: single connection per server lifetime), so the user re-runs launch from scratch. *Post-MVP design intent:* the underlying server keeps running so long as the SLURM job lives, the client attempts tunnel re-establishment up to a configurable cap (default 3 over 30 s), and on successful reconnect re-authenticates with the rendezvous token and resyncs state by requesting a snapshot.
- **Login node reboot.** Equivalent to SSH drop plus sshd unavailability. Session is declared lost regardless of v1/post-MVP reconnect status. If variant C/D, the compute-node server exits when its controlling SSH dies only if `srun` was the parent — which it is by default — so walltime is returned.
...
- **I12. Single connection per server lifetime in v1.** A `griz-server` process accepts exactly one client connection. If that connection drops (SSH tunnel death, client crash, network blip), the server exits and the user must relaunch — re-running launch and reloading the analysis from scratch. Reconnect-to-live-server is **explicitly out of scope for the MVP**; revisit post-v1 once disconnect frequency is observed in practice. Rationale: holds v1 scope tight, avoids designing the disconnect/reconnect state machine, idle-timeout policy, and concurrent-connection-rejection logic before there is evidence they are worth their weight. The 32-byte rendezvous token is therefore single-use per server lifetime; rotation is moot for v1. *Future intent (when reconnect ships post-MVP):* the same token is reused on reconnect — no rotation, no rekey events. Security boundary stays the same: rendezvous-file confidentiality (0600 in `$HOME`, owner-only). Threat-model rationale: an attacker with read access to the user's `$HOME` already has SSH keys and data, so token rotation does not meaningfully raise the bar.
